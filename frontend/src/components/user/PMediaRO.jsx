...
function PMediaRO() {
    //to get pmedia ro list from backend and show in table
    const [pmediaRO, setPmediaRO] = useState([]);
    function loadData() {
        axios.get(import.meta.env.VITE_BASEURL + "/pmediaros")
            .then((res) => {
                setPmediaRO(res.data);
            })
            .catch((error) => console.error("Error fetching pmdiaro:", error));
    }
    ...
    const resetFilter = () => {
        ...
        document.getElementById("paper").value = "Select Paper";
    ...
    function handleDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(import.meta.env.VITE_BASEURL + "/pmediaros/" + id)
                    .then((res) => {
                        console.log(res.data);
                ...
                <div className="container shadow pb-2">
                    ...
                    <div className="row g-3 align-items-end mt-2 mb-4">
                        ...
                        <div className="col-md-4 col-sm-6 text-end">
                            ...
                            <Link to="/p-media-ro/new-pmedia-ro" className="btn btn-success me-2">Add New RO</Link>
                        </div>
                    </div>
                </div>
                ...
                <div className="container mt-5">
                    <div className="card shadow-lg card-responsive ">
                        {/* <div className="card-header bg-white text-dark text-center my-3">
                            <h5 className="fw-bold">P-Media RO List</h5>
                        </div> */}
                                    ...
                                    <tbody>
                                        {
                                            pmediaRO.map((eachData, index) => (
                                                <tr key={eachData.id}>
                                                    ...
                                                    <td>{eachData.pmediaId}</td>
                                                    <td>{eachData.edition}</td>
                                                    <td>{eachData.scheme}</td>
                                                    <td>{eachData.roAmount}</td>
                                                    <td>{eachData.noOfRecords}</td>
                                                    ...
                                                    <td>
                                                        <div className="d-flex justify-content-center">
                                                            {/* Edit */}
                                                            <Link to={"/p-media-ro/new-pmedia-ro/" + eachData.id} className="btn btn-sm btn-primary me-2" style={{ height: "30px", width: "30px" }}><i className="fas fa-edit"></i></Link>
                                                            {/* Delete */}
